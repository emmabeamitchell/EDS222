...
1.  What potential values can a $Normal$ variable take? What about a $Bernoulli$ variable?
    ...
    normal can take - infinity to infinity , bernoulli variable is only 0 and 1
...
2.  What's the difference between a probability *density* function (PDF) and a probability *mass* function (PMF)?
    ...
    1.  Consider distributions from the $Normal$ and $Bernoulli$ families. Which has a PDF? Which a PMF?
        ...
        pmf is discrete variables, pdf is continuous

        pmf is bernoulli, pdf is normal
...
library(tidyverse)
# Create the data frame for Bernoulli(p=0.25)
...
ggplot(bernoulli_df, aes(x = factor(x), y = probability)) +
  geom_bar(stat = "identity", fill = "blue", width = 0.5) +
  labs(
    title = "PMF of Bernoulli(p=0.25) Variable",
    x = "Outcome",
    y = "Probability"
  ) +
  theme_minimal()
...
1.  The PMF of a random discrete variable describes the probability of observing a value given the parameters. What is the likelihood function relative to that?
    ...
    For each observed data point x, we compute its probability under the PMF for parameter θ.

    The product of these probabilities for all observations forms the likelihood function.
...
3.  Consider this logistic regression model from the week 6 lab.
    ...
    water_data <- tibble(pctHisp = seq(0, .9, by = .15),
