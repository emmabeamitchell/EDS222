...
2.  What does the intercept tell you, in words?
    ...
    the intercept is saying that when the engine displacement is zero, the highway miles per gallon will be 35.7922 (intercept refers to reference variable which is 1999)

3.  What does the coefficient on `displ` tell you, in words?

    the coefficient of displ is the slope of 1999 which is -3.7684

4.  What does the coefficient estimate on the 2008 vintage indicator variable (`year2008`) tell you, in words?

    year2008 is the intercept of 2008 is in relation to 1999. The coefficient is 0.3445 means that the intercept is 0.3445 bigger than the reference

5.  What does the coefficient on `displ:year2008` tell you, in words?[^1]

    displ:year2008 is the slope in reference to the reference variable. So .3052 as the coefficient means that the slope is .3052 larger for 2008 than 1999
...
```{r}
mod3 <- augment(mod2)
  ggplot(data = mod3, aes(x = displ, y = hwy, color = year)) +
           geom_point() +
           geom_abline(slope = coef(mod2)["displ"] +
                       coef(mod2))["displ:year2008"],
                        intercept = coef(mod2)["year2008"]) +
