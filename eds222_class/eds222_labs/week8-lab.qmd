...
nestling <- read_csv("C:/meds/EDS222-stats/eds222_class/eds222_labs/mockingbird_data/NestlingPb.csv") |> 
  filter(hood %in% c("uptown", "lakeshore"))

NOMO_Nest <- read_csv("C:/meds/EDS222-stats/eds222_class/eds222_labs/mockingbird_data/NOMO_Nest_data.csv") |>
...
ggplot(NOMO_Nest, aes(hood, fill = factor(bin.status))) + # using factor to tell ggplot it's a discrete value
  geom_bar() +
...
ggplot(nestling, aes(hood, blood_ug_dl_pbwt)) + 
  geom_boxplot() 
```

#### Step 1: state the null and alternative hypotheses

*H~0~: neighborhood has no effect on nestling blood Pb levels
H~A~:* neighborhood has an effect on nestling blood Pb levels
...
lead_diff <- mean_pb$mean[2] - mean_pb$mean[1]
...
ggplot(tibble(mean_pb), aes(mean_pb)) + 
  geom_histogram(bins = 20,
                 color = "cornflowerblue",
                 fill = NA) +
