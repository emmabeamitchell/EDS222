...
drinking_water <- read_csv(here("eds222_class", "eds222_labs", "drinking_water.csv"), show_col_types = FALSE)
...
2.  What columns do you think represent:
    ...
    2.  Percent Black and Hispanic population in the utility district? **pctblack, pcthisp**
...
4.  Create a scatter plot of violations against race (percent Black population), ethnicity (percent Hispanic population), and SES (median household income). What visualization issues do we get with a scatter plot? How could you address that?\
    \*\* the points are overplotted, ie too many are overlapping to see the trend. We can summarize within bins to make the pattern pop \*\*
...
drinking_water_complete <- 
  drop_na(drinking_water, pcthisp, violation) 
...
coef_optim
beta0_hat <- -2.422124762 
beta1_hat <- 0.004398142
...
3.  What's the predicted probability of drinking water violations for communities with 0%, 50%, and 100% Hispanic population?\
    Plot the predicted probability across the whole range 0-100% Hispanic.

```{r}
# Create and plot predictions
beta0 +beta1 * pcthisp
violations_zero <- inv_logit(-2.422124762 + 0.004398142 * 0)
violations_zero
violations_fifty <- inv_logit(-2.422124762 + 0.004398142 * 50)
violations_fifty
violations_hundred <- inv_logit(-2.422124762 + 0.004398142 * 100)
violations_hundred
...
tibble(pcthisp = 0:100,
       logitP = beta0_hat + beta1_hat *pcthisp,
       p = inv_logit(logitP)) %>% 
...
```{r}
inv_logit(-2.422124762 + 0.004398142 * 10) - inv_logit(-2.422124762 + 0.004398142 * 20)

inv_logit(-2.422124762 + 0.004398142 * 45) - inv_logit(-2.422124762 + 0.004398142 * 55)

inv_logit(-2.422124762 + 0.004398142 * 80) - inv_logit(-2.422124762 + 0.004398142 * 90)
```
...
likelihood_dem <- expand_grid(
  ...
  beta1 = seq(0.0, 0.01, length.out = 20)) |>
...
tells us that our model is very sensitive to changing intercept, but not sensitive to beta1 (slope) change
...
predictions <- expand_grid(
  ...
  medianincomehouse = seq(2000, 10000, length.out = 100)
) %>% 
